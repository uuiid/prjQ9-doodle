...
class myRemesh(object):
    ...
    def __init__(self):
        ...
        maya.cmds.floatField(annotation='Subdivision value', minValue=0.1, maxValue=10, step=0.05, value=1,
                             dragCommand=self._getMaxEdgeLength, changeCommand=self._getMaxEdgeLength)
        maya.cmds.floatField(annotation='Collapse Threshold', minValue=1, maxValue=80, step=1, value=1,
        ...
        self.file_path_lable_butten = maya.cmds.button(label="export file", command=self._selectObj)
        maya.cmds.setParent("..")
    ...
    def _MYremesh(self, t):
        ...
        for mesh in self.meshs:
            mesh_dub = maya.cmds.duplicate(mesh, name="{}_rems".format(mesh))[0]
    ...
    def _copyBindSkin(self, f):
        for index, b in enumerate(self.duplicate_mesh):
            ...
            maya.cmds.copySkinWeights(sourceSkin=soure_sk, destinationSkin=trange_sk, noMirror=False,
                                      surfaceAssociation="closestPoint", influenceAssociation="closestJoint")

    def _selectObj(self, f):
