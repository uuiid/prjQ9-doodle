...
class convertSet(object):
    ...
    def toCommand(self):
        list_com = ["DemBones.exe", "--abc", self.Iabc_filepath, "--init", self.Ifbx_filepath, "--out", self.ofbx_path,
...
class DleClothToFbx(QtWidgets.QMainWindow, UiFile.DleClothToFbx.Ui_MainWindow):
    ...
    def _getSelectMesh(self, a0):
        ...
        for line, tran in enumerate(self.tran):
            ...
            bem_bone.Ofbx_filename = tran.name()
            # 获得路径
            path = os.path.join(self.cache_path, self.getpath())
            # 填写输出路径和名称
            print(path)
            bem_bone.Ifbx_filename = tran.name() + u"_fbx"
            bem_bone.Ifbx_path = path

            bem_bone.Iabc_filename = tran.name() + u"_abc"
    ...
    def exportButtenClicked(self):
        self.client.send_bytes(pickle.dumps(
            ...
            "info":dict({"filepath":[bem.Iabc_path for bem in self.bem_bone] + 
                            [bem.Ifbx_path for bem in self.bem_bone]},**self.pathInfo()),
            "_data_":[bem.toCommand for bem in self.bem_bone]))
        # for t in self.bem_bone:
        #     t.exportMesh(self.ScaneStartFrame, self.ScaneEndFrame)
